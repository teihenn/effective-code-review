...
5. tmpコミットを「減算機能の追加」コミットの直後に移動し、pickをsquash(またはs)に変更する
...
6. エディタを保存して閉じると、新しいエディタが開き、コミットメッセージの編集画面になる(初期状態では、まとめようとしているコミット全てのメッセージが記載されている)。必要に応じて修正し、保存して閉じる。
...
9. pushする際は、リモートにpush済みのものに対しrebaseした場合は強制pushする必要が有る

```text
git push --force-with-lease origin feature/xxx
```

ちなみにsquashではなくeditやfixupでも出来る。
- edit(e): rebase中に該当ファイルを修正する
- fixup(f): squashと同じで複数のコミットを統合するが、新しいコミットメッセージを編集する機会は無く、統合元のコミットのメッセージが残る
